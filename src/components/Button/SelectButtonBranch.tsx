...
const RenderItem = ({
  ...
  const isSelected = data.findIndex((x) => x.branch_Name === value) === index;
  return (
    <TouchableOpacity
      activeOpacity={0.75}
    ...
    >
      ...
      >
        <Box flex={1} style={{ paddingRight: 20 }}>
          <Text>{Capitalize(item?.branch_Name)}</Text>
        </Box>
...
export default SelectButtonBranch;
